...
namespace H24
{
    ...
    public class QuoteWrapper
    {
        ...
        public static void GetTickerValues(ILogger logger, IOptions<AppSettings> settings, Dictionary<string, float> tickers)
        {
            ...
            try
            {
                ...
                foreach (string ticker in tickers.Keys.ToList())
                {
                    ...
                    {
                        string uri = string.Format(settings.Value.QuoteUri, ticker);
                        ...
                        logger.LogData("QuoteWrapper", Guid.NewGuid().ToString(), "Read {tickerValueDecoded} for {ticker}");
                    }
            ...
            {
                logger.LogData("QuoteWrapper", Guid.NewGuid().ToString(), $"Unable to read tickers: {ex.Message}");
            }
        }
    }
}
